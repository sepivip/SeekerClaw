...
- Secrets are now stripped from the Logs screen and from anything shared out of it. API keys and tokens are masked as log lines are collected and again as older entries are re-loaded, so they no longer appear on screen or in the one-tap Share output — previously redaction ran only on the Node side, leaving lines written directly by the app unmasked. Note that this covers what is displayed and shared; a log file saved by an earlier version keeps its original contents on disk until it is aged out by normal log rotation. (BAT-1161)
