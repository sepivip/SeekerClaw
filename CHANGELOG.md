...
- **Agent runtime crash on device startup** — `silent-reply.js` used Unicode property escapes (`\p{L}\p{N}`) in character classes, which parsed fine on desktop Node 22 but crashed nodejs-mobile's Node 18 V8 build at module load with `Invalid property name in character class`. The uncaught `SyntaxError` took down the whole agent, which manifested as `Node runtime failed to initialize` and a single-start lockout. Rewrote the boundary regexes to use ASCII `\w` instead (#325)
- **SILENT_REPLY sentinel over-stripped agent protocol discussion** — when a user asked the agent about the silent-reply protocol, the aggressive strip regex ate legitimate mentions of the token from the reply body, producing sentences with missing leading words and mid-sentence double spaces. Renamed the canonical sentinel from `SILENT_REPLY` to `[[SILENT_REPLY]]` (double-bracketed, Wiki/Obsidian-link style) so discussion of the protocol in prose uses the bare form and passes through untouched, while the bracketed form is reserved exclusively for the control signal. Kept a legacy whole-message-only compatibility path for bare `SILENT_REPLY` and its markdown-wrapped variants so older agents are still honored. 43 unit tests across 6 coverage sections lock in the behavior (BAT-491, #324)
- **Silent Replies system prompt over-priming** — the Silent Replies section in `buildSystemBlocks()` shipped explicit `✅ Right / ❌ Wrong` examples that contained the literal sentinel string. Seeing those examples in its own system prompt taught the agent to reproduce the literal form when describing the protocol, which the strip then ate from discussion. Rewrote the section so the literal form appears exactly once (on the "How to emit the signal" line) and added a dedicated subsection telling the agent to describe the protocol in natural language without writing the literal control string in any user-visible reply. Cron and heartbeat-path prompts no longer inline the literal form either — they reference the Silent Replies section by name (BAT-492, #326)
