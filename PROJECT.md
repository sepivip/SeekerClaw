...
| 2026-08-28 | Fix: **preserve the turn anchor against history trim** (BAT-1186 Stage 1) — on a long single turn the role-blind `MAX_HISTORY` front-trim could evict the current user instruction, so the model received a payload with no user turn and confabulated or abandoned the task mid-way. Reproduced on device on two providers. Non-anchor history is now capped and the in-flight anchor exempted, group-atomically so no tool pair is orphaned. | #446 |
...
| 2026-07-20 | **Release: v2.2.0** — sign in with your Grok subscription, the newest OpenAI models, and a security & logging hardening pass: xAI Grok as a full provider with subscription OAuth + restart-durable token refresh (BAT-1124, BAT-1155), the GPT-5.6 family with Sol as the OpenAI default (BAT-1151), timestamped + bounded diagnostic logs with Kotlin-side secret redaction (BAT-1161), truthful xAI error messages (BAT-1172), and the `web_fetch` / `agent_settings` hardening wave (BAT-1086/1087/1088). | #434, #442, #445, #433 |
