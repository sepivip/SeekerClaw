...
| Date | Feature | PR |
|------|---------|-----|
| 2026-07-12 | Feat: **Jupiter Trigger V2 is now the default** for limit/stop orders (BAT-1091) — USD-denominated trigger prices + explicit expiry; burner cap-routing / confirmations unchanged; V1 kept as an env kill-switch fallback (`SEEKERCLAW_USE_TRIGGER_V2=false`); ambiguous-create recovery via order-history + a DIAGNOSTICS entry + an **opt-in, test-only** live `api.jup.ag` contract-drift probe (dev harness under `tests/live/`, not run by the app). Slated for the next release (**version bump happens at release, not in this PR**). The bundled MWA 2.1.x / compileSdk 36 bump (BAT-1092) was deferred — the whole MWA 2.1.x line needs a Kotlin 2.0→2.2 toolchain upgrade (BAT-1149). | BAT-1148 |
