...
- **`unsupported.json` v2** — 63 known-but-not-usable entries with **six reason buckets** in the top-level `reasons` registry: `mpp_protocol`, `siwx_auth_required`, `invalid_demand`, `requires_binary_response`, `endpoint_not_402_at_probe`, `unverified_paid_response_shape`. Many entries carry an `audit_pending[]` field listing sibling endpoints found by the BAT-706 full-catalog audit (208 sibling endpoints total across 9 services — quicknode 133, stablesocial 36, stablecrypto extras, perplexity, fal, etc.) — each pending endpoint points at a BAT-XXX follow-up ticket (BAT-766/768/769/770/771/772/764) for catalog promotion. Agent can answer "I know about Google Vision but can't pay it — our probe got HTTP 400" honestly instead of generic "I don't have that."
