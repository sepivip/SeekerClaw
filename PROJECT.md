...
- **`unsupported.json`** — 62 known-but-not-usable services with **six reason buckets**: `mpp_protocol`, `siwx_auth_required`, `invalid_demand`, `requires_binary_response`, `endpoint_not_402_at_probe`, and `unverified_paid_response_shape` (the last added in PR #378 R10 for paysponge/screenshotone — openapi declares application/json contradicting product-family inference; held out pending paid-response capture under BAT-708). Full pay.sh catalog coverage (10 supported + 62 unsupported = 72 services). Some entries also carry BAT-706-audit notes about **sibling endpoints on the same host** that ARE payable but aren't yet catalogued (e.g. paysponge/perplexity has 3 paid endpoints, paysponge/nyne has 2). Agent can answer "I know about Google Vision but can't pay it — our probe got HTTP 400" honestly instead of generic "I don't have that."
