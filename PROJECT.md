...
- **`agent_pay(url, max_usdc)`** — pay an x402-protected HTTP endpoint and fetch its response.
  - **Security envelope (the SeekerClaw V1 boundary, unrelated to the x402 protocol version):** HTTPS GET only, Solana mainnet only, USDC only, private-IP rejection, DNS rebinding defense, max 1 MB response, 30s timeout. pay.sh-compatible.
  - **x402 protocol version support:** detect + build accept both **x402 v1** (`maxAmountRequired`, bare `network: "solana"`) and **x402 v2** (`amount`, CAIP-2 `network: "solana:<genesis>"`, `payment-required` header delivery, multi-chain pick-Solana). **Settle is v1-only currently** — v2 challenges are detected and built but settlement rejects with `v2_settle_not_implemented` until a real-wire v2 success-response capture pins the v2 proof-header path (BAT-582 Phase 5 follow-up). v3+ rejected as `unsupported_version`.
...
- **`paysh-catalog` skill** — folder-shaped catalog mapping user intent → known pay.sh service URL + cost + example. Agent reads `catalog.json` to pick a service, then reads only the matching `services/<name>.md` for query construction. 10 verified-payable services: Wolfram Alpha, Tripadvisor, 2Captcha, Rentcast, Reducto, Crushrewards, StableCrypto New Pools, StableEnrich, Purch, Textbelt SMS. Skill is OPT-IN ONLY (activates on explicit pay.sh / paysh / x402 / 'pay for' keywords) per BAT-704.
- **`unsupported.json`** — 62 known-but-not-usable services with reasons (mpp_protocol, siwx_auth_required, invalid_demand, requires_binary_response, endpoint_not_402_at_probe). Full pay.sh catalog coverage (10 supported + 62 unsupported = 72 services). Agent can answer "I know about Google Vision but can't pay it — our probe got HTTP 400" honestly instead of generic "I don't have that."
