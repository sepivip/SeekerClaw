...
- **9 catalog entries** in v2 (one per supported endpoint, mostly one per service today): Wolfram Alpha, Tripadvisor, 2Captcha, Rentcast, Reducto, Crushrewards, StableCrypto New Pools, Purch, Textbelt SMS. (StableEnrich was demoted to unsupported in BAT-761 PR #379 R7 — the v1 catalog claimed POST `/api/google-maps/place-details/partial` but the captured 402 was GET and the BAT-706 audit shows GET on that path returns http_400; needs re-probe with POST + real body in BAT-708 / BAT-772 before re-promotion.) Skill is OPT-IN ONLY (activates on explicit pay.sh / paysh / x402 / 'pay for' keywords) per BAT-704.
- **`unsupported.json` v2** — 63 known-but-not-usable entries with **six reason buckets** in the top-level `reasons` registry: `mpp_protocol`, `siwx_auth_required`, `invalid_demand`, `requires_binary_response`, `endpoint_not_402_at_probe`, `unverified_paid_response_shape`. Many entries carry an `audit_pending[]` field listing sibling endpoints found by the BAT-706 full-catalog audit (240 sibling endpoints total across 10 services — quicknode 133, stablesocial 36, stableenrich 32 [from the demoted catalog entry — see Tier 2c BAT-772], stablecrypto extras, perplexity, fal, etc.) — each pending endpoint points at a BAT-XXX follow-up ticket (BAT-766/768/769/770/771/772/764) for catalog promotion. Agent can answer "I know about Google Vision but can't pay it — our probe got HTTP 400" honestly instead of generic "I don't have that."
