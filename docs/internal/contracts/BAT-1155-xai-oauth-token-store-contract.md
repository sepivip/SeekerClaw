...
> **Status:** DRAFT v3 — v2 was reviewed by Codex ("not signing off v2 yet; four correctness amendments"). This v3 folds in all four (cross-process CAS lock, post-migration fail-closed, background-scoped notify, shutdown single-flight+budget) and adopts Codex's five decisions (§8). Back to Codex for sign-off. No implementation until sign-off + owner OK.
...
- **Budgets (recompute all three together):** `/shutdown/flush` runs inside `SeekerClawService.flushNodeBeforeProcessKill`'s `withTimeoutOrNull(2000ms)` (`706-712`) before the unconditional `killProcess` (`685`); the summary alone can use ~1200ms inside the `NodeControlClient` 1500ms read / 2000ms outer ceilings (`internal-control-server.js:404-427`). Adding a drain without **rebalancing all three** (Node token+summary sub-bounds, `NodeControlClient` connect/read timeouts, `SeekerClawService` outer timeout) makes the guarantee illusory. Give the token drain a reserved slice + **bypass the `5/60s` rate limit for the shutdown write**; the summary takes the remainder; widen the outer timeout if the arithmetic doesn't close.
...
- **Durable migration tombstone/marker, independent of successfully decoding the token record (Codex amendment 2):**
  ...
  - **marker present:** the dedicated store is the **only** source of truth. Store corruption / decrypt failure **FAILS CLOSED to reauth** (surface reconnect, set `reauthRequired`) — it **must NEVER emit a legacy token**. (This is the critical fix: a post-migration legacy fallback would replay an already-consumed refresh token — the exact incident — because after the first store-backed rotation the legacy prefs token is consumed.)
  - Run the idempotent migration/scrub gate in **both** processes: **MAIN before its first config load/save**, and **`:node` before the first `config.json` emit** — so no boot reads an empty store (→ api_key downgrade / "Missing required config" crash).
  - **Gate on the marker (or "store has no token AND legacy prefs has a token"), NEVER on file presence** (an empty/corrupt partial write must not masquerade as migrated).
...
7. **Shutdown single-flight + budget:** with a rotation persist **already in flight** when USER_STOP arrives, `flushPendingPersist` does **not** double-POST the same payload (no false CAS conflict); the token drain runs **before** the summary and completes within the recomputed ceiling.
