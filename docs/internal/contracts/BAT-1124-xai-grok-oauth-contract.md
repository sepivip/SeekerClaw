...
- **C1 (critical)** — `xai.js classifyError`: 403 is NOT `type:'auth'`; real retry-once via a module one-shot flag (not `MAX_RETRIES`). *Test: 403 never triggers refresh; retry bounded to one.* (See §5.4 correction.)
