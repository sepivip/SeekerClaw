...
**Secondary hooks:** Crypto-native (wallet, swaps, DCA, autonomous USDC payments + on-chain trading), device control, privacy (runs locally)
...
Powered by Claude, OpenAI, OpenRouter, or xAI Grok (100+ models) with persistent memory,
...
- 64 built-in tools with analytics
- Natural language cron scheduling
- Multi-provider web search + page reading
- MCP server support for extensible tools
- 37 skills with export/import — CalClaw calorie tracker, Netwatch security audit, and more
...
<!-- REVIEW 2026-07-12: Two big shifts since the last website pass are NOT yet reflected above the fold:
     (1) The v2 thesis is "your phone's AI can autonomously PAY for paid APIs in USDC" (burner wallet + x402)
         and now TRADE on-chain within caps. This is the strongest, most differentiated hook we have —
         it belongs in the hero + as a Feature Card, likely REPLACING the weaker "Modular Skill System" card.
     (2) 🏆 Solana Mobile Hackathon Winner (April 2026) — PROJECT.md leads with it; the site still omits it.
         Add a hero badge + a short Award/Recognition section above Features (announcement:
         https://x.com/RadiantsDAO/status/2049549148395798847).
     Also: the Shipped list is now ~22 items — overdue for a prune to ~8 user-facing wins before deploy. -->
