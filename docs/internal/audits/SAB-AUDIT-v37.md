...
> **Date:** 2026-06-27
> **SAB Version:** v3
> **Scope:** Pre-merge gate for BAT-1066 (CodeRabbit #413). The `solana_swap` `expectedDelta`-build `catch` previously flipped a **burner** route to `main` and signed via MWA — handing a burner-taker order to the main wallet (the BAT-1038 Amendment 1 risk class). Fix fails closed on a burner route with a new `expected_delta_unbuildable` error (retryable); main routes are unchanged. Touched `buildSystemBlocks()` (extended the burner tool-handler-errors door), `DIAGNOSTICS.md` (new tool-handler-errors row), a new tool error code + new WARN log site in `tools/solana.js`.
