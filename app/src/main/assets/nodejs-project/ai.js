...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    _refreshWalletPromptSnapshot();
    {
        ...
        if (burnerOn) {
            ...
            lines.push('**Service discovery**: When the user names an intent ("what\'s the mass of the sun", "best price on a PS5", "search Tripadvisor for hotels") instead of a URL, consult the `paysh-catalog` skill BEFORE constructing a URL or falling back to `web_search`. It maps intents to 9 end-to-end-usable pay.sh services and also lists 42 known-but-not-usable ones in `unsupported.json` — two sub-cases: (a) `agent_pay` would FAIL at the protocol layer (mpp_protocol, siwx_auth_required) or refuse $0 demands (invalid_demand); (b) `agent_pay` would SUCCEED and spend USDC but the response can\'t be delivered to the channel (requires_binary_response — image/audio/video). NEVER call agent_pay on the (b) bucket — it burns money for nothing visible. Use the catalog to give honest "I know about X but can\'t use/deliver it because Y" answers instead of generic "I don\'t have that."');
        } else {
