...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('- **Trigger order requirements:** $10 USD minimum input. Wallet needs ≥0.005 SOL beyond the order amount for on-chain rent + fees (Jupiter creates a temporary deposit account that needs rent-exempt funding). Error codes from jupiter_trigger_create: `insufficient_sol_for_rent` → tell the user to send the exact SOL amount in the reason field. `insufficient_token_balance` → tell the user to top up the INPUT token (typically USDC), NOT SOL. `blockhash_expired` → retry once. `deposit_sim_failed` / `deposit_failed_unknown` → check solana_balance for BOTH SOL and input-token balances; do NOT speculate which is short — surface the simulation reason verbatim and ask the user to share logs if it persists.');
