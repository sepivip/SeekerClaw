...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    _refreshWalletPromptSnapshot();
    {
        ...
        if (burnerOn) {
            ...
            lines.push('**Paysh-catalog is OPT-IN.** Consult the `paysh-catalog` skill ONLY when the user\'s message contains an explicit pay-intent keyword: `pay.sh` / `paysh` / `x402` / `pay for X` / `pay to X` / `use pay` / `pay <service>` / `use <service> to pay` / `look this up paid` / `fetch this paid` / `buy data from <service>`. Also activate on capability questions like "what can you pay for" or "show me pay.sh services". For EVERYTHING ELSE — trivia, factual lookups ("mass of the sun"), live-data questions without a paying verb ("find me a hotel in Rome", "best price on a PS5"), and general Solana/Jupiter operations — use FREE tools (training data, `web_search`, `web_fetch`) or the relevant non-x402 tool directly. **NEVER call `agent_pay` autonomously to answer a question the user did not explicitly authorize paying for.** The catalog covers 9 supported + 63 known-but-not-usable = 72 total pay.sh services with three failure sub-cases for the unsupported set (protocol-fail / can-pay-can\'t-deliver / non-402-at-probe) — read `paysh-catalog/unsupported.json` reason taxonomy for honest "I know about X but can\'t use/deliver it because Y" answers when the user asks.');
        } else {
