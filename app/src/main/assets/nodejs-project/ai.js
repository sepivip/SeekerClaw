...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    _refreshWalletPromptSnapshot();
    {
        ...
        if (burnerOn) {
            ...
            lines.push('**Paid APIs (x402)**: Use `agent_pay(url, max_usdc, method?, body?)` to fetch x402-protected endpoints (e.g., pay.sh catalog services). Settles in USDC from the **Burner wallet**. Mainnet only, HTTPS only. Default `method` is GET; pass `method: "POST"` + `body: <JSON-serializable>` for paid POST endpoints (≤ 8 KB body). GET runs silently when under cap. **POST always asks for user confirmation** (side-effect-aware: POST can send SMS, post content, or trigger paid actions). `max_usdc` is YOUR willingness ceiling per call; the BURNER CAP is the user\'s hard ceiling. Both bound the actual server demand, NOT max_usdc itself — so `max_usdc: "1.00"` against a $0.01 endpoint with a $0.10 cap pays $0.01. If a user sets a low cap "to test it," explain this before paying — to actually exercise cap rejection, they need a service whose demand exceeds the cap. If agent_pay returns `error: "insufficient_burner_balance"`, the reason text states the exact shortfall — tell the user how much more USDC to send to the burner pubkey and offer to retry once funded; do NOT retry blindly with higher max_usdc (max_usdc isn\'t the constraint, the on-chain balance is).');
            // BAT-699: paysh-catalog skill door. When the user names INTENT
            // (not a URL), the catalog skill maps it to a known-payable
            // endpoint. Also lists 63 known-but-not-usable services
            // (full upstream pay.sh catalog coverage: 9 supported + 63
            // unsupported = 72) so the agent can answer "do you know X?"
            // honestly. See app/src/main/assets/default-skills/paysh-catalog/
            // unsupported.json for the reason taxonomy.
            lines.push('**Service discovery**: When the user names an intent ("what\'s the mass of the sun", "best price on a PS5", "search Tripadvisor for hotels") instead of a URL, consult the `paysh-catalog` skill BEFORE constructing a URL or falling back to `web_search`. It maps intents to 9 end-to-end-usable pay.sh services and also lists 63 known-but-not-usable ones in `unsupported.json` — three sub-cases: (a) `agent_pay` would FAIL at the protocol layer (mpp_protocol, siwx_auth_required, invalid_demand); (b) `agent_pay` would SUCCEED and spend USDC but the response can\'t be delivered (requires_binary_response — image/audio/video); (c) endpoint was non-402 at last probe (endpoint_not_402_at_probe — broken, moved, or auth-gated differently). NEVER call agent_pay on bucket (b) — it burns money for nothing visible. Use the catalog to give honest "I know about X but can\'t use/deliver it because Y" answers instead of generic "I don\'t have that." Total catalog coverage: 9 supported + 63 unsupported = 72 pay.sh services.');
        } else {
