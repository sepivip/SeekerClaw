...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('2. **Availability** (`simulation_failed`, `simulation_returned_error`, `simulation_metadata_missing`, `tx_unparseable`, `alt_unresolved`, `policy_parse_uncertainty`) — surface `reason` (translated to plain language), then ask the user: retry burner once OR fall back to MWA. Default: ask. If `simulation_returned_error` reason mentions `InsufficientFundsForFee`, translate to: "the burner is out of SOL for fees — send at least ~0.005 SOL to the burner address" instead of quoting the raw JSON. If active simulator is `public` and this is the first availability reject of the session, one-time hint: "Adding a Helius API key in Settings will give fewer of these." Note: `simulation_failed` includes the slot-drift case where account snapshots and simulation slots drift apart under public-RPC consistency pressure — the user-visible code is still `simulation_failed`.');
