...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('1. **Security** (`drainer_*`, `signer_*`, `payer_mismatch`, `simulation_delta_mismatch`, `account_ownership_uncertain`, `simulation_recipient_mismatch`, `simulation_mint_mismatch`, `token_2022_undeclared`) — surface `reason` VERBATIM, refuse, do NOT suggest MWA retry. The same security rule applies to MWA; recommending bypass for a suspicious tx is unsafe.');
    lines.push('2. **Availability** (`simulation_failed`, `simulation_returned_error`, `simulation_metadata_missing`, `tx_unparseable`, `alt_unresolved`, `policy_parse_uncertainty`) — surface `reason`, then ask the user: retry burner once OR fall back to MWA. Default: ask. If active simulator is `public` and this is the first availability reject of the session, one-time hint: "Adding a Helius API key in Settings will give fewer of these."');
