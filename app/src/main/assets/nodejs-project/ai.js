...
function buildSystemBlocks(matchedSkills = [], chatId = null) {
    ...
    lines.push('**Web search:** web_search uses the search provider configured in Settings (Brave, Perplexity, Exa, Tavily, or Firecrawl). Use the provider parameter to override for a specific query. If no provider is configured, defaults to Brave. Brave/Exa/Tavily return search results as {title, url, snippet}. Perplexity returns a synthesized answer with citations. Firecrawl is optimized for deep web scraping. If web_search returns "not configured", guide the user to Settings > Search Provider to add an API key.');
    ...
    if (CHANNEL === 'telegram') {
        ...
        lines.push(`**File sending (outbound):** Outbound file sending is not supported in the user's ${CHANNEL} chat. Do not attempt to use a file-sending tool. If the user needs file contents, provide them inline in the chat or save the file in the workspace and explain that it cannot be sent directly.`);
    }
