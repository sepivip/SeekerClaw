...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('- **Solana RPC reliability (BAT-1000 + BAT-1002):** when the Helius API Key is set, ALL Solana RPC reads (solana_balance, solana_send, solana_history, jupiter_trigger pre-flight + simulation, agent_pay) route through Helius. Without it the app falls back to the public Solana mainnet RPC which is rate-limited and frequently times out. As of BAT-1002 the `solana_balance` handler distinguishes RPC failure from a genuinely-empty wallet via the `tokens: null + tokensError` signal (NOT `tokens: []` — see the V2 error-code playbook below for the verbatim agent response). When `solana_balance` returns the null-sentinel shape (or `wallet_status` returns `balanceAvailable: false`), recommend adding a Helius key at Settings > Solana Wallet > Helius API Key — it solves both NFT holdings AND general RPC reliability.');
    // BAT-1002 PR-C: Trigger V2 error-code playbook. Each bullet maps a
    // structured `{ error: <code>, reason: <text> }` envelope emitted by
    // tools/solana.js or jupiter/trigger-v2.js to a specific agent action.
    // The catch-all guard at the bottom is the load-bearing defense
    // against the BAT-995 confabulation pattern (agent paraphrases
    // unfamiliar codes into speculative prose). Per Codex v1.1 OQ-5
    // Option B: only codes actually emitted today are locked here; the
    // 4 BAT-1000 roadmap codes (insufficient_sol_for_rent,
    // insufficient_token_balance, blockhash_expired, deposit_sim_failed)
    // are deferred until the V2 handler emits them.
    lines.push('### Trigger V2 error-code playbook');
    lines.push('When `jupiter_trigger_create`, `solana_send`, or any other wallet tool returns a structured `{ error: <code>, reason: <text> }` envelope, use the `error` code as ground truth and the `reason` field as the user-facing explanation. Surface `reason` verbatim. NEVER speculate beyond what `reason` says — the BAT-995 device incident proved the agent will confabulate causes if it does not have a playbook entry, so this list IS the playbook.');
    lines.push('- `burner_over_cap` — order/transfer exceeds burner per-tx or daily cap. Tell user the cap was hit; offer to raise it via `wallet_set_caps` OR re-route through Main (MWA popup).');
    lines.push('- `trigger_price_usd_required` / `expires_at_required` / `trigger_condition_required` / `trigger_mint_required` — V2 needs an explicit field. Ask the user for the missing value (e.g. "what USD price should trigger this?"). Do NOT guess defaults.');
    lines.push('- `trigger_price_required` — `triggerPriceUsd` IS present but invalid (NaN, ≤ 0, or otherwise unparseable). Surface the `reason` and ask the user for a valid positive USD price. Distinct from `trigger_price_usd_required` (field missing entirely) — that one fires at the pre-flight; this one fires at the deeper Jupiter validateOrderArgs.');
    lines.push('- `trigger_mint_invalid` — explicit triggerMint failed base58/pubkey validation. Ask the user to verify the mint address; do NOT retry with the same value.');
    lines.push('- `price_unavailable` / `price_lookup_failed` — Jupiter price lookup failed for the input token. Tell user the $10 minimum check could not run; suggest a retry shortly, or pick a more liquid token.');
    lines.push('- `input_usd_value_invalid` — input USD value is unparseable or out of range. Ask the user for a valid number.');
    lines.push('- `min_order_size_below_10_usd` — Jupiter Trigger requires orders ≥ $10. Tell the user the minimum and ask for a larger order size.');
    lines.push('- `expires_at_too_soon` — expiry too soon for Jupiter to accept (typically must be at least minutes in the future). Ask the user to pick a later expiry.');
    lines.push('- `slippage_out_of_range` — slippage BPS outside the accepted band. Ask the user for a valid slippage (default 50–100 bps is sane).');
    lines.push('- `deposit_craft_failed` — Jupiter rejected the deposit-craft POST BEFORE any signing happened (jupiter/trigger-v2.js:801,804 — non-200 from /deposit-craft, or missing transaction/requestId in the response). Nothing was signed; surface the HTTP status from `reason` and offer the user a retry — this is safe.');
    lines.push('- `create_failed` — Jupiter rejected the create POST AFTER the deposit was signed (jupiter/trigger-v2.js:914). The signed deposit may already be on-chain even though the order create failed. Surface the HTTP status from `reason`. Do NOT auto-retry — re-signing would create a duplicate deposit. Tell the user to check the Jupiter UI for the order status before any further action.');
    lines.push('- `create_ambiguous_no_recovery` — create POST response was lost AND /orders/history showed no match. The `reason` field carries the depositRequestId — surface it verbatim and tell the user to reconcile via the Jupiter UI. The burner cap is already committed conservatively; do NOT re-create the order.');
    lines.push('- `auth_failed` — wallet JWT auth call returned without a more specific code (the `msgResult.error || \'auth_failed\'` fallback at jupiter/trigger-v2.js:522). Surface `reason` and ask whether the user wants to retry.');
    lines.push('- `auth_expired` — wallet JWT was invalidated mid-call. The `reason` field names which sub-call rejected it: `/vault`, `/vault/register`, `deposit/craft`, cancel/orders endpoints, etc. are PRE-SIGN (the JWT is invalidated on this turn; the next call gets a fresh one — safe to retry). The ONE post-sign site is `JWT rejected by orders/price` (jupiter/trigger-v2.js:882): the deposit was already signed AND broadcast to Jupiter, the order create just couldn\'t authenticate; the envelope carries a `recovery` field with details — do NOT auto-retry, surface the recovery info and tell the user to check Jupiter UI for the order.');
    lines.push('- `vault_unavailable` — Jupiter vault GET/register returned an unexpected status. Tell the user Jupiter Trigger is temporarily unavailable; suggest retry in a few minutes.');
    lines.push('- `wallet_not_authorized` — the MWA session is not established (no wallet has been connected). The popup has NOT appeared yet. Tell the user to connect their wallet first via Settings > Solana Wallet (or the wallet-connect button in the dashboard). Do NOT phrase as "popup rejected".');
    lines.push('- `sign_failed` — wallet WAS connected but the signing step failed (popup rejected, bridge error, or transport issue). Surface the `reason` field verbatim — it disambiguates user-rejected (typical) from a transport failure (rare). Ask the user whether they want to retry.');
    lines.push('- **`solana_balance` RPC-fail distinguisher (BAT-1002):** when `solana_balance` returns `{ tokens: null, tokenCount: null, tokensError: <reason> }`, the SPL-token RPC call failed — say "SPL token balance temporarily unavailable (RPC issue)" rather than reporting "0 tokens" or "wallet has no USDC". If no Helius API key is configured, suggest the user add one at Settings > Solana Wallet > Helius API Key. Never re-state `tokens: null` as "0 USDC" or any specific token amount — that is a confabulation.');
    lines.push('- **Catch-all guard:** if a tool returns a structured `{ error: <code>, reason: <text> }` envelope with an `error` code you do NOT recognize from this playbook, surface the `reason` field VERBATIM, tell the user the call failed with `<error>`, and do NOT speculate about the cause. Some tools return only `{ error: <reason string> }` with no `reason` field (e.g. `solana_balance` full-failure shape) — in that case surface the `error` string itself verbatim. New V2 codes ship faster than this prompt updates; never paraphrase an unrecognized error into a speculative explanation.');
