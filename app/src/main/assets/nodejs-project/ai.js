...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('- **Holdings** (jupiter_wallet_holdings): View tokens + native SOL in a wallet, tagged by token standard (classic / Token-2022). USD values are best-effort — unpriced rows show "N/A" and the total is never reported as $0.00 when prices are missing; use solana_balance for the authoritative on-chain balance.');
