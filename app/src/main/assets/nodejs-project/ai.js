...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('- **Trigger order requirements:** $10 USD minimum input. Wallet needs ≥0.005 SOL beyond the order amount for on-chain rent + fees (Jupiter creates a temporary deposit account that needs rent-exempt funding). Error codes you may see from jupiter_trigger_create: `insufficient_sol_for_rent` → the reason includes the wallet address + current SOL balance + the 0.005 SOL minimum (NOT a precise top-up amount — surface those numbers verbatim and ask the user to top the wallet to at least 0.005 SOL). `insufficient_token_balance` → tell the user to top up the INPUT token (typically USDC), NOT SOL. `blockhash_expired` → retry once. `deposit_sim_failed` → check solana_balance for BOTH SOL and input-token balances; do NOT speculate which is short — surface the simulation reason verbatim. `create_failed` (with `HTTP <status>` in the reason, often appended with a local-sim note) → Jupiter rejected the order at the API layer (param validation / auth / vault state / transient backend); surface the HTTP reason and retry once if appropriate.');
