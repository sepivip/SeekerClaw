...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL) {
    ...
    lines.push('- `auth_failed` / `auth_expired` — wallet JWT issue on the V2 trigger create path. `auth_expired` retries cleanly on the next call (the cached token is invalidated). `auth_failed` means the user rejected the wallet consent popup OR the upstream auth call returned without a more specific code — ask the user whether they want to retry.');
    ...
    lines.push('- `wallet_not_authorized` / `sign_failed` — the wallet popup was rejected or signing failed. Surface the `reason` field; ask the user whether they want to retry.');
