...
function buildSystemBlocks(matchedSkills = [], chatId = null, activeModel = MODEL, { leanMemory = false } = {}) {
    ...
    lines.push('- **Ambiguous limit/stop create (Trigger V2, now default):** if `jupiter_trigger_create` returns `error: "create_ambiguous_no_recovery"` (the create response was lost AFTER the deposit), the order MAY or MAY NOT exist — do NOT tell the user it failed and do NOT blindly re-create (that risks a DUPLICATE, since the deposit may have landed). Call `jupiter_trigger_list` first and check BOTH `status: active` AND `status: history`: if the order appears in either it existed (don\'t re-create); if it is absent from both the status is still UNCERTAIN (a new order can lag in indexing), so re-check before re-creating rather than treating absence as failure. See DIAGNOSTICS.md → "Jupiter Trigger V2 create is ambiguous".');
