...
const tools = [
    ...
    {
        name: 'daily_note',
        description: 'Record an event or conversation tied to today\'s date in the daily memory file. Use this for things that HAPPENED, not for durable facts about entities or for the user\'s stable preferences. If the event also includes a durable update about a named entity (e.g. "Shipped BAT-990 today" — event AND project-entity update), ALSO call `notebook_save` in parallel. Do NOT use this for the user\'s long-running preferences (use `memory_save` instead) or for facts about named entities like people, projects, tokens, places, topics, or tools (use `notebook_save` instead). Same secrets policy as `memory_save` — NEVER pass API keys, OAuth tokens, seed phrases, private keys, passwords, or auth headers through this tool; daily notes are plain-text on disk AND indexed into the SQL search DB. For API keys the user provides in chat, save to agent_settings.json under apiKeys.<service>; for higher-risk secrets (seed phrases, private keys, etc.), direct the user to the Settings UI (which routes each secret class to the appropriate secure storage backend).',
        input_schema: {
            type: 'object',
            properties: {
