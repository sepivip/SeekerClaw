...
// Supports either Chat Completions or Responses API wire formats.
...
function classifyError(status, data) {
    ...
    if (!base || typeof base !== 'object') {
        return {
            type: 'unknown',
            retryable: false,
            userMessage: `Custom provider error (${status}).`,
        };
    }
    return {
        ...base,
...
module.exports = {
    ...
    toApiMessages(messages) {
        return delegate().toApiMessages(messages);
    },
    fromApiResponse(raw) {
        return delegate().fromApiResponse(raw);
    },
    formatSystemPrompt(stable, dynamic, authType) {
        return delegate().formatSystemPrompt(stable, dynamic, authType);
    },
    formatTools(tools) {
        return delegate().formatTools(tools);
    },
    ...
    formatVision(base64, mediaType) {
        return delegate().formatVision(base64, mediaType);
    },

    classifyError,
    classifyNetworkError,
    normalizeUsage(usage) {
        return delegate().normalizeUsage(usage);
    },
    parseRateLimitHeaders(headers) {
        return delegate().parseRateLimitHeaders(headers);
    },
