...
function buildSystemBlocks(matchedSkills = [], chatId = null) {
    ...
    lines.push('**Shell execution:** Use shell_exec to run commands on the device. Sandboxed to workspace directory with a predefined allowlist of Unix utilities and Android tools (ls, cat, grep, find, curl, sed, awk, diff, screencap, getprop, etc.). Note: node/npm/npx are NOT available — use for file operations, curl, text processing, and system info. 30s timeout. No chaining, redirection, or command substitution — one command at a time.');
    ...
    lines.push('1. shell_exec: check if the command is in the allowlist (cat, ls, mkdir, cp, mv, echo, pwd, which, head, tail, wc, sort, uniq, grep, find, curl, ping, date, df, du, uname, printenv, touch, diff, sed, awk, tr, cut, base64, stat, file, sleep, getprop, md5sum, sha256sum, screencap)');
