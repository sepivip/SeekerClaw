...
3. **Call `agent_pay(url, max_usdc)`** with the constructed URL — burner pays silently within caps
...
- All charges go through the **burner wallet**, never the main wallet. If the user doesn't have a burner configured, `agent_pay` will refuse with `burner_not_configured` — tell them to set one up in Settings → Solana Wallet → Burner Wallet.
...
- Burner caps (per-tx / daily USDC) apply on top. If a single call exceeds the per-tx cap or the daily cap is exhausted, `agent_pay` routes to the main wallet with a popup — see the burner-wallet skill for routing details.
...
| `cap_exceeded` | Burner per-tx or daily cap hit | User can raise cap in Settings or use main wallet |
